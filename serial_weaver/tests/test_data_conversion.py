...
from serial_weaver.exceptions import SerialWeaverDumperError
...
def dst_file(request):
    return f"serial_weaver/tests/mocked_data/_customers.{request.param}"
...
def test_format_conversion(src_file, dst_file):
    if src_file != dst_file:
        ...
        if dst_format == 'ini':
            with pytest.raises(SerialWeaverDumperError):
            ...
            with open(dst_file, "r") as f_created:
                expected_file = f"serial_weaver/tests/mocked_data/customers.{dst_format}"
